...
import { syncUserProfile } from '../../services/ServerSyncService';
...
const EditProfileScreen = ({ route }) => {
  ...
  try {
    if (formData.profileImage) {
      if (typeof formData.profileImage === 'string') {
        // Ensure the source is properly formatted as an object with uri property
        profileImageSource = { uri: formData.profileImage };
      } else if (typeof formData.profileImage === 'object' && formData.profileImage.uri) {
        // If it's already an object with uri property, use it directly
        profileImageSource = formData.profileImage;
      } else {
        // Fallback to default image if profile image is in an unexpected format
  ...
  // Handle updating profile image and saving to database
  const handleImagePickerResult = async (pickerResult) => {
    if (!pickerResult.canceled && pickerResult.assets && pickerResult.assets.length > 0) {
      try {
        ...
        // 1. Save to database through user context
        const contextSuccess = await updateUser(newData);
        if (!contextSuccess) {
          console.error("Failed to update user through context");
        }
        
        // 2. Save updated image to user data key for persistence
        try {
          const userDataKey = `userData_${formData.email.toLowerCase().trim()}`;
          const storedData = await AsyncStorage.getItem(userDataKey);
          
          if (storedData) {
            const userData = JSON.parse(storedData);
            userData.profileImage = permanentImageUri;
            await AsyncStorage.setItem(userDataKey, JSON.stringify(userData));
            console.log("Permanent profile image URI saved to user data:", userData.email);
            ...
            await AsyncStorage.setItem(`userHasProfileImage_${formData.email.toLowerCase().trim()}`, "true");
            await AsyncStorage.setItem(`userProfileImagePath_${formData.email.toLowerCase().trim()}`, permanentImageUri);
            
            // Force an immediate backup of the entire user data 
            const updatedUserData = { 
              ...userData, 
              profileImage: permanentImageUri,
              updatedAt: new Date().toISOString()
            };
            await updateUser(updatedUserData);
          } else {
            console.error("No user data found for direct update");
          }
        } catch (storageError) {
          console.error("Failed to directly update user data in storage:", storageError);
        }
  ...
  const createPermanentImageCopy = async (sourceUri, userEmail) => {
    try {
      ...
      const filename = `profile_${userEmail.replace(/[^a-zA-Z0-9]/g, '_')}_${timestamp}.jpg`;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.log("Saving user profile with image:", userToSave.profileImage ? "YES" : "NO");
      ...
      // First update through context
      const success = await updateUser(userToSave);
      
      if (success) {
        // Additionally, ensure profile image and other data is directly saved to AsyncStorage for extra redundancy
        const userDataKey = `userData_${userToSave.email.toLowerCase().trim()}`;
        const storedData = await AsyncStorage.getItem(userDataKey);
        ...
        if (storedData) {
          ...
          userData.name = userToSave.name;
          userData.phone = userToSave.phone;
          userData.age = userToSave.age;
          userData.address = userToSave.address;
          ...
          if (userToSave.profileImage) {
            userData.profileImage = userToSave.profileImage;
          }
          
          // Save updated user data
          await AsyncStorage.setItem(userDataKey, JSON.stringify(userData));
          console.log("User data directly saved during full profile update");
          ...
          // Also save profile image backup flags if needed
          if (userToSave.profileImage) {
            await AsyncStorage.setItem(`userHasProfileImage_${userToSave.email.toLowerCase().trim()}`, "true");
            await AsyncStorage.setItem(`userProfileImagePath_${userToSave.email.toLowerCase().trim()}`, userToSave.profileImage);
          }
        }
        ...
        try {
          ...
          const syncResult = await syncUserProfile(formData.email.toLowerCase().trim());
          if (syncResult.success) {
            console.log("Profile synced successfully with server");
          } else {
            console.log("Profile sync failed, but local updates were successful");
          }
        } catch (syncError) {
          console.error("Error syncing profile with server:", syncError);
        ...
        throw new Error("Failed to update user data through context");
      }
    } catch (error) {
